const DB_NAME='mikhael-v4', DB_VER=4, STORE='profiles';
const META='meta';
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
const DEFAULT={day:1,completedDays:[],sessions:0,activeDays:[],streak:0,lastActive:null,skills:{comprension:0,escucha:0,lectura:0,produccion:0,gramatica:0},cards:{},history:[],settings:{largeText:false,reduceMotion:false,haptic:true},readerDone:{},readerStats:{},courseStartedAt:null};
let DATA={alphabet:[],vocab:[],phrases:[],grammar:[],course:[],reader:[],vocalization:[],plans:[],morphology:[]};
let state=structuredClone(DEFAULT); let currentProfileId=null; let currentProfileName=''; let currentView='home'; let lesson=null;
let review={queue:[],cursor:0,ratings:{},path:[]}; let deferredInstall=null;

async function loadData(){
 const files=['alphabet','vocabulary','phrases','grammar','course','reader','vocalization','lesson-plans','morphology'];
 const vals=await Promise.all(files.map(f=>fetch(`./data/${f}.json`).then(r=>{if(!r.ok)throw Error(f);return r.json()})));
 files.forEach((f,i)=>{const k=f==='vocabulary'?'vocab':f==='lesson-plans'?'plans':f;DATA[k]=vals[i]});
}
function openDB(){return new Promise((res,rej)=>{const r=indexedDB.open(DB_NAME,DB_VER);r.onupgradeneeded=()=>{const db=r.result;if(!db.objectStoreNames.contains(STORE))db.createObjectStore(STORE);if(!db.objectStoreNames.contains(META))db.createObjectStore(META)};r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error)})}
async function metaGet(key){try{const db=await openDB();return new Promise((res,rej)=>{const q=db.transaction(META).objectStore(META).get(key);q.onsuccess=()=>res(q.result);q.onerror=()=>rej(q.error)})}catch{return null}}
async function metaPut(key,val){const db=await openDB();return new Promise((res,rej)=>{const q=db.transaction(META,'readwrite').objectStore(META).put(val,key);q.onsuccess=res;q.onerror=()=>rej(q.error)})}
async function dbGet(){if(!currentProfileId)return null;try{const db=await openDB();return new Promise((res,rej)=>{const q=db.transaction(STORE).objectStore(STORE).get(currentProfileId);q.onsuccess=()=>res(q.result||null);q.onerror=()=>rej(q.error)})}catch{return null}}
async function dbPut(){const db=await openDB();return new Promise((res,rej)=>{const q=db.transaction(STORE,'readwrite').objectStore(STORE).put(state,currentProfileId);q.onsuccess=res;q.onerror=()=>rej(q.error)})}
async function profiles(){return (await metaGet('profiles'))||[]}
async function saveProfiles(p){await metaPut('profiles',p)}
async function ensureProfile(){let p=await profiles();let active=await metaGet('activeProfile');if(!p.length){const id='p_'+Date.now();p=[{id,name:'Mi perfil',createdAt:Date.now()}];active=id;await saveProfiles(p);await metaPut('activeProfile',active)}currentProfileId=active||p[0].id;const x=p.find(x=>x.id===currentProfileId)||p[0];currentProfileId=x.id;currentProfileName=x.name}
async function switchProfile(id){const p=await profiles(),x=p.find(x=>x.id===id);if(!x)return;currentProfileId=id;currentProfileName=x.name;await metaPut('activeProfile',id);const old=await dbGet();state=old?{...structuredClone(DEFAULT),...old,skills:{...DEFAULT.skills,...old.skills},settings:{...DEFAULT.settings,...old.settings}}:structuredClone(DEFAULT);lesson=null;review={queue:[],cursor:0,ratings:{},path:[]};applySettings();renderProfilePanel();show('home')}
async function createProfile(name){name=(name||'').trim().slice(0,28);if(!name){alert('Escribe un nombre para el perfil.');return}const p=await profiles();if(p.some(x=>x.name.toLowerCase()===name.toLowerCase())){alert('Ya existe un perfil con ese nombre.');return}const x={id:'p_'+Date.now(),name,createdAt:Date.now()};p.push(x);await saveProfiles(p);await metaPut('activeProfile',x.id);currentProfileId=x.id;currentProfileName=x.name;state=structuredClone(DEFAULT);await dbPut();renderProfilePanel();show('home')}
async function resetCurrentProfile(){if(!confirm(`¿Restablecer por completo el progreso de ${currentProfileName}? Esta acción no se puede deshacer.`))return;state=structuredClone(DEFAULT);lesson=null;review={queue:[],cursor:0,ratings:{},path:[]};await save();show('home')}
async function deleteCurrentProfile(){const p=await profiles();if(p.length<=1){alert('Debe existir al menos un perfil. Puedes restablecer este perfil en su lugar.');return}if(!confirm(`¿Eliminar el perfil ${currentProfileName} y todo su progreso local?`))return;const db=await openDB();await new Promise((res,rej)=>{const q=db.transaction(STORE,'readwrite').objectStore(STORE).delete(currentProfileId);q.onsuccess=res;q.onerror=()=>rej(q.error)});const rest=p.filter(x=>x.id!==currentProfileId);await saveProfiles(rest);await metaPut('activeProfile',rest[0].id);await switchProfile(rest[0].id)}
function renderProfilePanel(){const host=$('#profilePanel');if(!host)return;profiles().then(p=>{host.innerHTML=`<div class="profile-box"><span class="eyebrow">Perfil local</span><h2>${currentProfileName||'Mi perfil'}</h2><p class="muted small">Cada perfil guarda por separado su curso, SRS, estadísticas y lecturas. No se requiere correo electrónico.</p><select id="profileSelect">${p.map(x=>`<option value="${x.id}" ${x.id===currentProfileId?'selected':''}>${x.name}</option>`).join('')}</select><div class="row"><input id="newProfileName" maxlength="28" placeholder="Nuevo usuario"><button class="secondary" id="createProfileBtn">Crear</button></div><div class="row"><button class="secondary" id="resetProfileBtn">Restablecer este perfil</button><button class="danger" id="deleteProfileBtn">Eliminar perfil</button></div></div>`;$('#profileSelect').onchange=e=>switchProfile(e.target.value);$('#createProfileBtn').onclick=()=>createProfile($('#newProfileName').value);$('#resetProfileBtn').onclick=resetCurrentProfile;$('#deleteProfileBtn').onclick=deleteCurrentProfile})}
...
function activeVocab(){return DATA.vocab.filter(v=>!v.deprecated)}
function introducedVocabIds(day=state.day){const ids=new Set();DATA.plans.slice(0,Math.max(1,Math.min(180,day))).forEach(p=>(p.unit_ids||[]).forEach(id=>ids.add(id)));return ids}
function curriculumVocab(day=state.day){const ids=introducedVocabIds(day);return activeVocab().filter(v=>ids.has(v.id))}
function dueCards(){const now=Date.now();return activeVocab().filter(v=>{const c=state.cards[v.id];return !!c&&!!c.due&&c.due<=now})}
function adaptiveQueue(limit=25,pool=activeVocab()){return [...pool].filter(v=>!!state.cards[v.id]).sort((a,b)=>scoreCard(b)-scoreCard(a)).slice(0,limit)}
function schedule(id,rating){
 const c=cardState(id),now=Date.now(); c.last=now; c.reps++;
 if(rating===0){ c.errors++; c.ease=Math.max(1.3,c.ease-.2); c.state='learning'; c.interval=c.reps<=2?.007:.04; }
 else if(c.state==='new'||c.state==='learning'){
   const steps=[.007,.04,1,3]; const step=Math.min(3,Math.max(0,c.learningStep||0)+(rating>=2?1:-1));
   c.learningStep=step; c.interval=steps[step]; c.state=step>=2?'review':'learning';
   c.ease=Math.max(1.3,c.ease+[-.12,-.04,.02,.12][rating]);
 } else {
   const mult=[.55,1.15,2.5,3.4][rating]; c.interval=Math.max(1,c.interval*mult*(c.ease/2.5));
   c.ease=Math.max(1.3,c.ease+[-.18,-.05,.03,.12][rating]); c.state='review';
 }
 c.due=now+c.interval*86400000; state.history.push({ts:now,id,rating,interval:c.interval,ease:c.ease});
...
function unitsForPlan(plan){
 const base=plan.unit_ids.map(id=>DATA.vocab.find(v=>v.id===id)).filter(v=>v&&!v.deprecated);
 const eligible=curriculumVocab(plan.day);
 const weak=adaptiveQueue(4,eligible);
 return [...new Map([...base,...weak].map(v=>[v.id,v])).values()]
   .sort((a,b)=>{const ai=base.findIndex(x=>x.id===a.id),bi=base.findIndex(x=>x.id===b.id);if(ai!==-1&&bi===-1)return -1;if(ai===-1&&bi!==-1)return 1;return (a.frequency_rank||99999)-(b.frequency_rank||99999)})
   .slice(0,6);
}
...
async function completeDay(){
 ...
 state.day=Math.min(180,state.day+1); state.sessions++; touchActivity(); state.skills.comprension+=1; state.skills.lectura+=1;
 if(!state.courseStartedAt)state.courseStartedAt=Date.now(); await save(); lesson=null; renderCourse(); show('home');
}
function beginReview(){if(review.queue.length)return;const due=dueCards().sort((a,b)=>scoreCard(b)-scoreCard(a));review.queue=due.slice(0,25);review.cursor=0;review.ratings={};review.path=[0]}
function currentReview(){return review.queue[review.cursor]}
function renderReview(){beginReview();const q=review.queue;if(!q.length){$('#reviewCount').textContent='0';$('#reviewCard').innerHTML='<div class="eyebrow">Todo al día</div><h2>No hay tarjetas pendientes.</h2><p class="muted">El SRS repasa únicamente unidades que ya han entrado en tu aprendizaje. Las unidades nuevas entran al SRS desde el Curso o cuando tú las añades desde el Lector.</p>';return}const v=currentReview(),c=cardState(v.id),rated=review.ratings[v.id]!==undefined,hasNext=review.cursor<q.length-1;$('#reviewCount').textContent=q.filter(x=>review.ratings[x.id]===undefined).length;$('#reviewCard').innerHTML=`<div class="eyebrow">Tarjeta ${review.cursor+1} de ${q.length}</div><div class="he">${v.hebrew}</div><div class="roman">${v.translit||''}</div><p id="meaning" class="meaning ${rated?'':'hidden'}">${v.meaning}</p><div class="review-actions"><button class="secondary" id="prev" ${review.cursor===0?'disabled':''}>← Anterior</button><button class="secondary" id="reveal">${rated?'Ver significado':'Revelar'}</button>${rated&&hasNext?'<button class="primary" id="nextReview">Siguiente →</button>':''}</div><div id="ratings" class="review-actions ${rated?'hidden':''}"><button class="danger" data-r="0">Otra vez</button><button class="secondary" data-r="1">Difícil</button><button class="secondary" data-r="2">Bien</button><button class="primary" data-r="3">Fácil</button></div>${rated?'<div class="notice"><b>Ya calificada.</b> Puedes volver a verla con «Anterior» y continuar con «Siguiente» sin generar otro repaso.</div>':''}<p class="muted small">Repasos: ${c.reps} · Errores: ${c.errors} · Intervalo: ${formatInterval(c.interval)}</p>`;$('#reveal').onclick=()=>$('#meaning').classList.remove('hidden');$('#prev').onclick=()=>{if(review.cursor>0){review.cursor--;renderReview()}};const next=$('#nextReview');if(next)next.onclick=()=>{if(review.cursor<q.length-1){review.cursor++;renderReview()}};$$('#ratings [data-r]').forEach(b=>b.onclick=()=>gradeReview(Number(b.dataset.r)))}
...
function tokenParts(text){return String(text).split('־').map(x=>x.trim()).filter(Boolean)}
function readerUnlockLevel(){return Math.min(5,Math.max(1,Math.ceil((state.day||1)/36)))}
function readerRecommendedText(){const unlocked=readerUnlockLevel();const exact=DATA.reader.filter(t=>t.level===unlocked);return exact[(state.day-1)%Math.max(1,exact.length)]||DATA.reader.find(t=>t.level===unlocked)||DATA.reader[0]}
function readerOpenText(id){return DATA.reader.find(t=>t.id===id)||readerRecommendedText()}
function readerTokenCount(t){return (t.tokens||[]).length}
function renderReader(){
 const unlocked=readerUnlockLevel();
 const labels={1:'inicio',2:'básico',3:'guiado',4:'intermedio',5:'independiente'};
 $('#readerLevels').innerHTML=[1,2,3,4,5].map(l=>{const locked=l>unlocked;return `<button class="secondary ${currentReaderLevel===l?'active':''}" data-level="${l}" ${locked?'disabled':''}>Nivel ${l}${locked?' · 🔒':''}</button>`}).join('');
 $$('#readerLevels [data-level]').forEach(b=>b.onclick=()=>{currentReaderLevel=Number(b.dataset.level);selectedReaderTextId=null;renderReader()});
 const texts=DATA.reader.filter(t=>t.level===currentReaderLevel).sort((a,b)=>(a.order||0)-(b.order||0));
 const recommended=readerRecommendedText();
 if(!selectedReaderTextId || !texts.some(t=>t.id===selectedReaderTextId))selectedReaderTextId=(recommended.level===currentReaderLevel?recommended.id:texts[0]?.id);
 const t=readerOpenText(selectedReaderTextId); if(!t){$('#readerText').innerHTML='<div class="card"><h2>No hay lecturas disponibles.</h2></div>';return;}
 const tokens=t.tokens||[]; const newCount=tokens.filter(tok=>{const v=vocabForToken(tok);return v&&!state.cards[v.id]}).length;
 ...
 $$('.readerword').forEach(b=>b.onclick=()=>{
   ...
   $$('.readerword').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');stats.words++;
   if(!v&&!m){$('#readerHelp').innerHTML=`<b>Ayuda contextual</b><p class="muted small">Esta forma todavía no tiene una ficha completa. Intenta identificar su función por el contexto.</p>`;return;}
   if(v)stats.help++;
...
function exportState(){const blob=new Blob([JSON.stringify({app:'Mikhael',version:'4.6',profile:{id:currentProfileId,name:currentProfileName},exportedAt:new Date().toISOString(),state},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='mikhael-progreso.json';a.click();URL.revokeObjectURL(a.href)}
...
function showProfileGate(){
 const gate=$('#profileGate'); if(!gate)return;
 profiles().then(p=>{
   gate.classList.remove('hidden');
   gate.innerHTML=`<div class="profile-gate-card"><span class="eyebrow">Mikhael</span><h1>¿Quién va a estudiar?</h1><p class="muted">Elige un perfil local. No necesitas correo electrónico. Cada perfil conserva su propio curso, SRS y estadísticas.</p><div class="gate-list">${p.map(x=>`<button class="profile-gate-option ${x.id===currentProfileId?'current':''}" data-profile-id="${x.id}"><span>${x.name}</span>${x.id===currentProfileId?'<small>perfil actual</small>':''}</button>`).join('')}</div><div class="row"><input id="gateNewProfile" maxlength="28" placeholder="Crear nuevo usuario"><button class="primary" id="gateCreateProfile">Crear</button></div><button class="secondary wide" id="gateContinue">Continuar con ${currentProfileName||'este perfil'}</button></div>`;
   $$('.profile-gate-option').forEach(b=>b.onclick=async()=>{await switchProfile(b.dataset.profileId);showProfileGate()});
   $('#gateCreateProfile').onclick=async()=>{const name=$('#gateNewProfile').value;await createProfile(name);showProfileGate()};
   $('#gateContinue').onclick=()=>gate.classList.add('hidden');
 });
}
